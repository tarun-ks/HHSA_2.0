...
<!--[Start] Add for R6.2  -->
	<input type='hidden' name='tokenStatus' id='tokenStatus' value="None" />
<!--[End] Add for R6.2  -->
	<%
	...
	if(null!=request.getAttribute("transactionStatus") && "passed".equalsIgnoreCase((String)request.getAttribute("transactionStatus"))){%>
		...
		<!--[Start] Add for R6.2  -->
		<script type="text/javascript">$("#tokenStatus").val("passed");</script>
		<!--[End] Add for R6.2  -->
	<%}else if(( null != request.getAttribute("transactionStatus") ) && "failed".equalsIgnoreCase((String)request.getAttribute("transactionStatus"))){%>
	     ...
	     <!--[Start] Add for R6.2  -->
	    <script type="text/javascript">$("#tokenStatus").val("failed");</script>
		<!--[End] Add for R6.2  -->
	<%}%>
		...
		<div class="">
			<div class='formcontainer'>
				...
				<div class="buttonholder">
					<input type="button"   id = "cancelshare1" class="graybtutton" value="Cancel" />
					<input type="submit" class="button" id="ResetId" title="Reset" value="Reset" />
				</div>
				
				<script type="text/javascript">
					var sts = $('#tokenStatus').val();
					if( sts == "failed" ) { $("#newNycIdPwd").attr('disabled', true);  $("#confirmNewPwd").attr('disabled', true);  $("#ResetId").attr('disabled', true); }
					else                  { $("#newNycIdPwd").attr('disabled', false); $("#confirmNewPwd").attr('disabled', false); $("#ResetId").attr('disabled', false); }
				</script>
...
$.validator.addMethod("passwd_policy", function( value, element, param ) {
	...
	$("#myPwdResetNewPwd").validate({  
                	...
                	submitHandler: function(form){
    	            	...
    	            	pageGreyOut();
						document.myPwdResetNewPwd.action=document.myPwdResetNewPwd.action+'&accoutRequestmodule=accoutRequestmodule&next_action=viaemailpwdresetToLoginScreen';                
