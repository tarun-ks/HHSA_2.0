...
<c:choose>
	<c:when test="${ProcurementCOF.contractTypeId != '2'}">
		<h2>Contract Certification of Funds</h2>
	</c:when>
	<c:otherwise>
		<h2>Contract Certification of Funds - Amendment</h2>
	</c:otherwise>
</c:choose>
...
<div class="formcontainer">
	 <div class="row">
	      ...
	      <span class="formfield">${ProcurementCOF.agencyName}</span>
	 </div>
	 <div class="row">
	      <span class="label clearLabel">Agency Code:</span>
	      <span class="formfield">${ProcurementCOF.agencyId}</span>
	  </div> 
    <c:if test="${ProcurementCOF.contractTypeId != '2'}">
	    <div class="row">
	      <span class="label clearLabel">Contract Value:</span>
	      <span class="formfield" ><label id="contractVal">${ProcurementCOF.contractValue}</label></span>
	    </div>  
    </c:if> 
    <%-- Start R 8.0.0 QC 9376 Display Vendor Name  on COF   --%>
    <div class="row">
	      <span class="label clearLabel">Provider:</span>
	      <span class="formfield" >${ProcurementCOF.providerName}</span>
	</div>
    ...
    --%>
   <%-- End R 8.0.0 QC 9376 Display Vendor Name  on COF   --%> 
    ...
    <c:if test="${ProcurementCOF.contractTypeId == '2'}">
    <div class="row">
      <span class="label clearLabel">Amendment Title:</span>
      <span class="formfield">${ProcurementCOF.amendmentTitle}</span>
    </div>
    </c:if>
    	<%--code updation for R4 ends--%>
	<div class="row">
      <span class="label clearLabel">Procurement E-PIN:</span>
      <span class="formfield">${ProcurementCOF.procEpin}</span>
    </div>
    <div class="row">
      <span class="label clearLabel">Award E-PIN:</span>
      <span class="formfield">${ProcurementCOF.awardEpin}</span>
    </div>
    	<%--code updation for R4 starts--%>
    <c:if test="${ProcurementCOF.contractTypeId == '2'}">
    <div class="row">
      <span class="label clearLabel">Amendment E-PIN:</span>
      <span class="formfield">${ProcurementCOF.amendmentEpin}</span>
    </div>
    </c:if>
	<div class="row">
      <span class="label clearLabel">Contract Term:</span>
      <span class="formfield">${ProcurementCOF.contractStartDate} - ${ProcurementCOF.contractEndDate}</span>
    </div>
    
     <c:if test="${ProcurementCOF.contractTypeId == '2'}">
    <div class="row">
      <span class="label clearLabel">Amendment Term:</span>
      <span class="formfield">${ProcurementCOF.amendmentStartDate} - ${ProcurementCOF.amendmentEndDate}</span>
    </div>
    	<%--code updation for R4 ends--%>
	    <div class="row">
	      <span class="label clearLabel">Procurement Value:</span>
	      <span class="formfield" ><label id="procVal">${ProcurementCOF.procurementValue}</label></span>
	    </div> 
	    
	     <div class="row">
	      <span class="label clearLabel">Contract Value:</span>
	      <span class="formfield" ><label id="contractValType2">${ProcurementCOF.contractValue}</label></span>
	    </div>
	    
	     <div class="row">
	      <span class="label clearLabel">Amendment Value:</span>
	      <span class="formfield" ><label id="amendmentValue">${ProcurementCOF.amendedContractValue}</label></span>
	    </div>
	...
	<div class="row">
      ...
      <span class="formfield">${ProcurementCOF.userFirstName} ${ProcurementCOF.userLastName}</span>
    </div>
	<div class="row">
      <span class="label clearLabel">Date Submitted:</span>
      <span class="formfield">${ProcurementCOF.modifiedDate}</span>
    </div>
	<div class="row">
      <span class="label clearLabel">Approved By:</span>
      <span class="formfield">${ProcurementCOF.approverFirstName} ${ProcurementCOF.approverLastName}</span>
    </div>
	<div class="row">
      <span class="label clearLabel">Date Approved:</span>
      <span class="formfield">${ProcurementCOF.approvedDate}</span>
    </div>
</div>
...
<c:choose>
	...
	<c:otherwise>
		<h3>Funding Source Allocation - Amendment</h3>
	</c:otherwise>
</c:choose>
<%-- Grid Starts --%>
		<div class="tabularWrapper" style="overflow-x:auto">
			 <table cellspacing='0' cellpadding='0' border="1">
				<tbody>
					...
					<tr class='oddRows'>
							...
							%>
							<td class='alignRht'>
							<span class= "tableData"><%=loFundingAllocationBean.getTotal()%></span>
							</td>
						</tr>
							<%
						 		}
...
.ready(
		function() {
			if('${ProcurementCOF.contractTypeId}' != 2){
				$("#contractVal").jqGridCurrency();	
			}
			else if('${ProcurementCOF.contractTypeId}' == 2){
				$("#procVal").jqGridCurrency();
				$("#contractValType2").jqGridCurrency();
				$("#amendmentValue").jqGridCurrency();
				$("#newContractValue").jqGridCurrency();
			}
			$(".nycgov_header").hide();
			$("#nyc_header_div").hide();
			$(".breadcrumb").hide();
