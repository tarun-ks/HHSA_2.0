...
<% 
	...
	boolean lbSecurityCheck=CommonUtil.getConditionalRoleDisplay(ComponentMappingConstant.TA_S080_PAGE, request.getSession());
...
%>
	...
	<%if(lbSecurityCheck){%>
		...
		<div class="tasktopfilter taskButtons">
			...
			<div class="taskreassign"><select id="finishtaskchild"
		...
		<div id="tabs">
			<ul>
				...
				<li><a href="#tabs-2" title="Add Section Comments">Add Section Comments</a></li>
				<li><a href="#tabs-3" title="View Task History">View Task History</a></li>
				 <li></li>
				 
				 <div class='floatRht'>
				 	<input id="save" name="save" type="button" class="graybtutton" title="Save" value="Save" onclick="saveComments()"/>
				 </div>
			</ul>
			...
			<div id="tabs-1" >
				...
				<!-- Grid Starts --> 
				<st:table
					objectName="loAssociatedDocs1" gridName="documenttable" cssClass="heading"
					alternateCss1="evenRows" alternateCss2="oddRows">
					<st:property headingName="Questions / Document Name"
						columnName="msQuestionDocumentName" align="center" size="15%">
							<st:extension
								decoratorClass="com.nyc.hhs.frameworks.sessiongrid.WorkFlowDocumentExtension" />
					</st:property>
					<input type="hidden" name="sections" value="msDocumentName">
					...
					<st:property headingName="Current Status" columnName="msCurrentStatus"
						align="right" size="15%" />
					...
					<st:property headingName="Last Modified By" columnName="msModifiedBy"
						align="right" size="15%" />
					<st:property headingName="Assign Status"
						columnName="msAssignedStatus" align="right" size="20%">
						<st:extension
							decoratorClass="com.nyc.hhs.frameworks.sessiongrid.DocumentStatusExtension" />
					</st:property>
				</st:table> 
				<!-- Grid Ends -->
				</div>
			...
			<label class="required"> * Do not save incomplete comments. The provider comments cannot be cleared - enter 'N/A' if necessary.<br>
				...
				<div class="taskComments"><b>Enter any public provider comment</b>
					...
					<% if((request.getAttribute("providerComments")==null) || (request.getAttribute("providerComments").equals(""))) {  %>
						...
						onchange="setChangeFlag()" cols="48" rows="5" onFocus="if (this.value == this.defaultValue) {this.value = '';}"></textarea>
					<% }else{ %> <textarea name="publicCommentArea"  class='taskCommentsTxtarea' id="publicCommentArea" onkeyup="setMaxLength(this,300)" onkeypress="setMaxLength(this,300)" 
						onchange="setChangeFlag()" cols="46" rows="5"><%=request.getAttribute("providerComments")%></textarea>
					<%}%>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if("null" != '<%= request.getAttribute("messagecom")%>'){
			$(".errorDiv").html('<%= request.getAttribute("messagecom")%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />");
			$(".errorDiv").addClass('passed');
			$(".errorDiv").show();
		 ...
		 **/
		 ...
		 {
			 document.getElementById('errorDiv').innerHTML = '<%=lsM57%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />";
			 $(".errorDiv").addClass('failed');
			 var element = document.getElementById('errorDiv');
		...
		if('<%=lsExceptionMsg%>'!= "null")
	 	{
			document.getElementById('errorDiv').innerHTML = '<%=lsExceptionMsg%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />";
			$(".errorDiv").addClass('failed');
			var element = document.getElementById('errorDiv');
			element.style.display ="block";
	 	}
	...
	/**
	 * Open Subsection and documents of Section Task in same page.
	 **/
	function openDocument(documentId,documentName){
		...
		if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_GEOGRAPHY%>'
				&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='basics' /><render:param name='subsection'  value='geography' /><render:param name='next_action'  value='open' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_LANGUAGES%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='basics' /><render:param name='subsection'  value='languages' /><render:param name='next_action'  value='open' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_POPULATIONS%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='basics' /><render:param name='subsection'  value='populations' /><render:param name='next_action'  value='open' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_QUESTION%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='basics' /><render:param name='subsection'  value='questions' /><render:param name='next_action'  value='showquestion' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_BOARD_QUESTION%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='board' /><render:param name='subsection'  value='questions' /><render:param name='next_action'  value='showquestion' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_FILINGS_QUESTION%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='filings' /><render:param name='subsection'  value='questions' /><render:param name='next_action'  value='showquestion' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		
		else if(documentName=='<%=P8Constants.PROPERTY_PE_BR_SUBSECTION_POLICIES_QUESTION%>'
			&& documentId==documentName){
			document.getElementById(documentId).className = "taskSelected";
			var url = "<render:standalonePortletUrl portletUri='/portlet/application/businessapplication/businessapplication.portlet'><render:param name='section' value='policies' /><render:param name='subsection'  value='questions' /><render:param name='next_action'  value='showquestion' /><render:param name='business_app_id'  value='<%=appId%>' /><render:param name='cityUserSearchProviderId'  value='<%=providerId%>' /><render:param name='removeNavigator'  value='true' /><render:param name='removeMenu'  value='true' /><render:param name='fromTaskDetails'  value='true' /></render:standalonePortletUrl>";
			previewUrl(url,'linkDiv');
		}
		else{
			document.getElementById(documentId).className = "taskSelected";
	...
	**/
	function checkUserTask(isManagerStep)
	{
		...
		if(isManagerStep=="true")
		{
			if(userName.split("|")[0]!="manager")
			{
				 ...
				 document.getElementById('errorDiv').innerHTML = '<%=lsM39%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />";
				 $(".errorDiv").addClass('failed');
				 var element = document.getElementById('errorDiv');
				 element.style.display ="block";
				 document.getElementById("reassign").disabled= true;
	...
	/**
	 * This method is upated for release 3.8.0 defect#5599
	 * Submit form on click on Save button.
	 **/
		function saveComments(){
				if('<%=lbSecurityCheck%>' && '<%=lsfromBRAppPage%>'=="true"){
					document.forms[0].action=document.forms[0].action+'&next_action=saveCommentsOrStatus'+'&fromBRAppPage=true';
					document.forms[0].submit();
				}
				else if('<%=lbSecurityCheck%>'){
		...
		function validateDocStatus(){
			var tmpCustomLabelReqError = true;
			 $(".selectReturned").each(function(){
			 var selectOpt = $(this).find(":selected").text();
			 if(selectOpt.trim() == ''){
				 $("#errorDiv").html("All questions and documents status should be marked to finish the task.");
				 $(".errorDiv").addClass('failed');
					var element = document.getElementById('errorDiv');
					element.style.display ="block";
				 tmpCustomLabelReqError =  false;
         ...
         .dialog({
               ...
               buttons: {
                     OK: function () {
                    		...
                    		if('<%=lsfromBRAppPage%>'=="true")
            				{
            					document.forms[0].action=document.forms[0].action+"&returninbox=BRApplication"+'&taskUnlock='+taskId+'&fromTaskManagementPage='+'<%=lsFromTaskManagementPage%>';
            					document.forms[0].submit();
            				}
            				else
            				{
	 ...
	 **/
	function finishCall(){
		var tmpIsError = validateDocStatus();
		 if(!tmpIsError){
		    	return false;
		   }
	 document.getElementById("finish").disabled= true;
	 var dropValue = document.getElementsByName('assignedstatus');
	 var isAllVerified="true";
	for (var i = 0; i <	 dropValue.length; i++)
	{
	var val = dropValue[i].options[dropValue[i].selectedIndex].innerHTML;
	if(val=="Returned")
	{
	isAllVerified="false";
	break;
		...
		if('<%=lbSecurityCheck%>' && document.getElementById('finishtaskchild').value!='' && !(<%=isTaskLocked%>)){
			if(isAllVerified=="false" ){
				if(document.getElementById('finishtaskchild').value=='<%=ApplicationConstants.STATUS_APPROVED%>')
				{
					...
					document.getElementById('errorDiv').innerHTML = '<%=lsM41%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />";
					$(".errorDiv").addClass('failed');
					var element = document.getElementById('errorDiv');
					element.style.display ="block";
				}
				else
				{
					if(newProviderComments=='' || newProviderComments=='<%=lsIntialCommentValue%>' )
					{
						...
						document.getElementById('errorDiv').innerHTML = '<%=lsM40%>'+"<img src=\"../framework/skins/hhsa/images/iconClose.jpg\" id=\"box\" class=\"message-close\" title=\"Close\" alt=\"Close\" onclick=\"showMe('errorDiv', this)\" />";
						$(".errorDiv").addClass('failed');
						var element = document.getElementById('errorDiv');
						element.style.display ="block";
					}
					else
					{
					 	...
					 	if('<%=lsfromBRAppPage%>'=="true")
						{
						document.myform.action=document.myform.action+'&next_action=finishchild&lsReturnToBrDetails=BRApplication';
						}
					 	else{
