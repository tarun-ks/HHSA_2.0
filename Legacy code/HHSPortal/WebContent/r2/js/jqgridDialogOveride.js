//This Overide the JQGrid's Error message
var oldInfoDialog = $.jgrid.info_dialog;
$.extend($.jgrid, {
 info_dialog: function (caption, content, c_b, modalopt) {
 	 
     if (modalopt && (modalopt.zIndex === null || modalopt.zIndex === undefined || (typeof modalopt.zIndex === "number" && modalopt.zIndex < 1234))) {
         modalopt.zIndex = 1234;
     }
     // set error message for mandatory field
     if (content.indexOf('Field is required') !== -1) 
     {
     	content = getHeaderName(headerName, content) + " is required.";
     } 
     // set error message for invalid number
     else if (content.indexOf('Please, enter valid number') > -1) 
     {
     	content = getHeaderName(headerName, content.replace('-','')) + " must be a number.";
     }
     // set error message for invalid Integer
     else if (content.indexOf('Please, enter valid integer value') > -1) 
     {
     	content = getHeaderName(headerName, content.substr(content.indexOf(' ')+1)) + " must be a number without decimal.";
     }
     // set error message to allow only negative values
     else if(content.indexOf('allowOnlyNegativeValue')>= 0)
 	{
     	if (msgKey != '') 
         {
             content = msgKey;
     	...
     	{
     		content = getHeaderName(headerName, content) + " must be a negative value.";
         }
 	}
     // set error message to allow only positive values
     else if(content.indexOf('allowOnlyPositiveValue')>= 0)
 	{
     	if (msgKey != '') 
         {
             content = msgKey;
     	...
     	{
     		content = getHeaderName(headerName, content.replace('-','')) + " must be a positive value.";
         }
 	}
     // set error message for duplicates values
     else if (content.indexOf('containDuplicateValues') !== -1)
     {
         content = "Entered value matches an existing row. Please enter a new value in the '" +getHeaderName(headerName, content.substring(content.replace(/ /g, "_").lastIndexOf('_') + 1))+ "' column.";
     } 
     // set error message for duplicates values in Other scenario
     else if (content.indexOf('containForOtherDuplicateValues') !== -1) 
     {
         content = "Entered value matches an existing row. Please enter a new value in the '" +getHeaderName(headerName, content.substring(content.replace(/ /g, "_").lastIndexOf('_') + 1))+ "' column.";
     } 
     // set error message for invalid Percent value
     else if (content.indexOf('IsNotAPercentageFormat') !== -1)
     {
         content = 'Please enter a percent value in '+getHeaderName(headerName, content)+"in the format 'NN.NN%'";
     } 
     // set error message for exceeded percentage value
     else if (content.indexOf('ExceedPercentageValue') !== -1) 
     {
         content = getHeaderName(headerName, content) + ' cannot exceed 100.00%';
     } 
     // set error message for invalid UoA*-BC*-OC*'s formatter
     else if (content.indexOf('valuesuobcinvalid') !== -1) 
     {
         content = 'UoA*-BC*-OC* must contain 11 characters.';
     }
     // set error message for amount $10,000,000,000,000,000.00
     else if (content.indexOf('amountgreaterthen10000000000000000') !== -1) 
     {
         content = "! Please enter a value less than $10,000,000,000,000,000.00";
     }
     // set error message for amount ($10,000,000,000,000,000.00)
     else if(content.indexOf('amountlesserthen10000000000000000') !== -1)
	 {
    	 content = "! Please enter a value greater than ($10,000,000,000,000,000.00)";
	 }
     // set error message for non-acceptable characters
     else if(content.indexOf('Non-Acceptable Characters') !== -1)
	 {
    	 content = getHeaderName(headerName, content) + " contains Non-Acceptable Characters.";
	 }
     return oldInfoDialog.call(this, caption, content, c_b, modalopt);
 }
});

//This method return the validation column's name
function getHeaderName(headerName, content) {
	content = $.trim(content.substring(0,content.indexOf(":")));
	var tempHeaderName = headerName.split('|');
	 for (var i = 0; i < tempHeaderName.length; i++) {
		 if(tempHeaderName[i] != ''){
			var checkVal = tempHeaderName[i].split(':');
			if (content.indexOf(checkVal[1]) !== -1) {
	 	        if(content.length == checkVal[1].length){
	 	        	return checkVal[0];
	 	        } 
	 	     }
			//Updated in Release 6
	 		else if (content.indexOf(checkVal[0]) !== -1 && $.trim(checkVal[0]) != "") {
		         return checkVal[0];
		     }
		 }
	 }
