...
public class ListOptionTag extends BodyTagSupport
{
	...
	public int doStartTag()
	{
		...
		try
		{
			ListOption loListOptionObj = new ListOption(value);
			if (selected != null && !selected.trim().equals(""))
			{
				loListOptionObj.setSelected(selected);
			}
			ListBoxTag loListBox = (ListBoxTag) findAncestorWithClass(this, ListBoxTag.class);
			...
			{
				loListBox.addListValue(loListOptionObj);
			}
		}
		catch (Exception aoExp)
		{
			LOG_OBJECT.Error("Error occured while generating List Option Tag", aoExp);
		}
		return (SKIP_BODY);
	}
}
