...
function openDocument(documentId, selectElement, documentName) {
	...
	if (value == 1) {
		...
		document.agencyform.action = agnFormAction
				+ '&documentId='
				+ documentId
...
function uploadDocument(formaction) {
	...
	document.agencyform.action = formaction;
}
...
function closeOverLay(agnFormAction) {
	pageGreyOut();
	$("#next_action").val('showdocumentlist');
	document.uploadform.action = agnFormAction
...
function selectAllDisplayDocuments() {
	if (document.agencyform.selectAllDocuments.checked = true && document.agencyform.selectAllDocuments.value == "Check All") {
		for ( var a = 0; a < document.agencyform.check.length; a++) {
			document.agencyform.check[a].checked = true;
		}
		document.agencyform.selectAllDocuments.value = "UncheckAll";
	} else {
		for ( var a = 0; a < document.agencyform.check.length; a++) {
			document.agencyform.check[a].checked = false;
		}
		document.agencyform.selectAllDocuments.value = "Check All";
	}
}
