 ...
 * 
 */
public class PreprocessingAL314 implements PreProcessingNotificationAlert
{
	private static final LogInfo LOG_OBJECT = new LogInfo(PreprocessingAL314.class);
	/**
	 * <ul>
			...
			throws ApplicationException
	{
		...
		try
		{
			...
			loHMap.put(HHSP8Constants.ENTITY_TYPE, HHSConstants.ENTITY_TYPE_CONTRACT_BUDGET_AMEND_REVIEW);
			loHMap.put(HHSP8Constants.ENTITY_ID, aoGroupNotificationBean.getEntityId());
			...
			aoGroupNotificationBean.setMessageBody(aoGroupNotificationBean.getMessageBody().replaceAll(
					"\\{#AGENCY_NAME\\}", lsAgencyName));
			lsAccComments = (String) DAOUtil.masterDAO(aoMyBatisSession, loHMap,
					ApplicationConstants.MAPPER_CLASS_NOTIFICATION_MAPPER, HHSConstants.FETCH_COMMENTS_AL314,
					HHSP8Constants.JAVA_UTIL_MAP);
			LOG_OBJECT.Info("Entered in fetch accelerator comments:" + lsAccComments);
			if (null != lsAccComments)
			{
				aoGroupNotificationBean.setMessageBody(aoGroupNotificationBean.getMessageBody().replaceAll(
						"\\{#ACCELERATOR_COMMENTS\\}", lsAccComments));
			}
			else{
				aoGroupNotificationBean.setMessageBody(aoGroupNotificationBean.getMessageBody().replaceAll(
						"\\{#ACCELERATOR_COMMENTS\\}", HHSConstants.EMPTY_STRING));
			}
		...
		{
			LOG_OBJECT.Error("Error occurred while Preprocessing AL314:", aoExp);
		}

		catch (Exception loExp)
		{
			LOG_OBJECT.Error("Error occurred while Preprocessing AL314:", loExp);
		}
