...
<script type="text/javascript" src="../resources/js/addContract.js"></script>
...
<%if(CommonUtil.getConditionalRoleDisplay(ComponentMappingConstant.SA_S070_S104_S070R_PAGE, request.getSession())){%>
	...
	<input type="hidden" id="contractHidden" value="${loReadOnlySection}"></>
	...
	<input type="hidden" value="<%=renderRequest.getAttribute("service_app_id")%>" id="hiddenServiceAppId" name="hiddenServiceAppId"/>
	<input type="hidden" value="<%=renderRequest.getAttribute("business_app_id")%>" id="hiddenBusinessAppId" name="hiddenBusinessAppId"/>
	<input type="hidden" value="<%=renderRequest.getAttribute("section")%>" id="hiddenSectionId" name="hiddenSectionId"/>
	<input type="hidden" value="<%=renderRequest.getAttribute("subsection")%>" id="hiddenSubSectionId" name="hiddenSubSectionId"/>
	...
	<div class="">
	    <h2 class='wordWrap'>Service: ${serviceName}</h2>
	    <!-- Container Starts -->
	    <div class="container"> 
			...
			<div id="mymain">
	                ...
	                <br />
               		...
               		%>
						...
						<input type="hidden" name="oldContractId" value="<%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractID())%>">
						<input type="hidden" name="oldProgramName" value="<%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractDescription())%>">
					...
					<span class="formfield">
	                    ...
	                    '<%=renderRequest.getAttribute("service_app_id")%>',
	                	'<%=renderRequest.getAttribute("business_app_id")%>','<%=renderRequest.getAttribute("section")%>',
	                	'<%=renderRequest.getAttribute("subsection")%>','${elementId}')" id="selectBox"  ${readOnlyValue}>
			  					...
			  					while(locontractIterator.hasNext()){
									...
									if(loContract.getMsContractDetailsId().equalsIgnoreCase((String)request.getAttribute("contractGrant"))){
										if(loContract.getMsContractType().equalsIgnoreCase("NYC Government")){
											%>
											<option selected="selected" value="<%=loContract.getMsContractDetailsId()%>" id="<%=loContract.getMsContractID()%>" title="<%=loContract.getMsContractNYCAgency()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%>"> 
											<%=loContract.getMsContractNYCAgency()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%></option>
										<%
										} else {
											%>
											<option selected="selected" value="<%=loContract.getMsContractDetailsId()%>" id="<%=loContract.getMsContractID()%>" title="<%=loContract.getMsContractFunderName()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%>">
										...
										if(loContract.getMsContractType().equalsIgnoreCase("NYC Government")){
											%>
											<option value="<%=loContract.getMsContractDetailsId()%>" title="<%=loContract.getMsContractNYCAgency()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%>"> <%=loContract.getMsContractNYCAgency()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%></option>
										<%
										} else {
											%>
											<option value="<%=loContract.getMsContractDetailsId()%>" title="<%=loContract.getMsContractFunderName()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%>"> <%=loContract.getMsContractFunderName()%> - <%=loContract.getMsContractID()%> - <%=loContract.getMsContractDescription()%></option>
										<%
										}
	            ...
	            <div class="formcontainer">
		                   		...
		                   		<c:forEach var="staffTitle" items="${staffTitle}">
		                    		<c:choose>
		                    			<c:when test="${reqContract.msContractRefTitle eq staffTitle.key}">
		                    				<option selected="selected" value="${staffTitle.key}" title="${staffTitle.value}">${staffTitle.value}</option>
		                    			</c:when>
		                    			<c:otherwise>
		                    				<option value="${staffTitle.key}" title="${staffTitle.value}">${staffTitle.value}</option>
		                    			</c:otherwise>
		                    		</c:choose>
								</c:forEach>
		                    </select>
	                ...
	                <div class="row"> 
	                	...
	                	<span class="formfield">
		                    ...
		                    <c:choose>
		                    	<c:when test="${not empty checkValue || checkValue ne ''}">
		                    		<input class="input" type="text"  id="msContractId" name="msContractId" maxlength="30" value="<%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractID()) %>"  ${readOnlyText}/>
		                    	</c:when>
		                    	<c:otherwise>
		                    		<input class="input" type="text" id="msContractId" name="msContractId" maxlength="30" value="<%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractID()) %>"  ${readOnlyText}/>
		                    	</c:otherwise>
                     ...
                     <%--  Below text area events are modified to fix defect 4453 as part of release 2.7.0--%>
	                <div class="row" id="contractDescriptionDiv" title="Add the name and brief description (no more than 250 characters) of the program you received contract or grant to deliver."> <span style ="height: 100px;" class="label"><span class="required">*</span>Program Name/Description:<p>Briefly describe how your program demonstrates experience in the selected service.</p></span> 
		                <span class="formfield">
		                    <textarea  onkeypress="setMaxLengthForAddContract(this,250)"  onkeyup="setMaxLengthForAddContract(this,250)"class="textarea floatLft input" id="contractDescription" name="contractDescription" style="display:block" value="<%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractDescription()) %>" ${readOnlyText}><%=StringEscapeUtils.escapeHtml((String)contractDetails.getMsContractDescription()) %></textarea>
		                </span>
	                    <span class="error"></span>
                    </div>
	                <div class="row" id="contractStartDateDiv"> <span class="label"><span class="required">*</span>Start Date:</span> <span class="formfield">
	                    ...
	                    <img src="../framework/skins/hhsa/images/calender.png" title="Start Date" onclick="NewCssCal('contractStartDate',event,'mmddyyyy');return false;"/>
	                ...
	                <div class="row" id="contractEndDateDiv"> <span class="label"><span class="required">*</span>End Date:</span> <span class="formfield">
	                    ...
	                    <img src="../framework/skins/hhsa/images/calender.png" title="End Date" onclick="NewCssCal('contractEndDate',event,'mmddyyyy');return false;"/>
	                    </span><span class="error"></span>
                    </div>
	           ...
	           <c:if test="${serviceComments ne null and ! empty serviceComments}">
	            ...
	            <div class="buttonholder" id="buttonSection">
	            		
				
	                <input id="cancelform" type="button" class="graybtutton" value="Cancel" title="Cancel" name="cancelform" onclick="cancel('<%=renderRequest.getAttribute("service_app_id")%>',
	                '<%=renderRequest.getAttribute("business_app_id")%>','<%=renderRequest.getAttribute("section")%>','<%=renderRequest.getAttribute("subsection")%>','${elementId}')"/>
	            <c:if test="${!loReadOnlySection}">
	                <input id='saveform' type="submit" title="Save" value="Save" name="saveform" onclick="checkContractIdValue()"/>		
	           	</c:if>		
	            </div>
            <!-- Form Data Ends --> 
			</div>
