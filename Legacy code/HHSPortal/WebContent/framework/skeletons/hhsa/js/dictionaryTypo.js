...
Typo.prototype = {
	...
	suggest : function (word, limit) {
		...
		/**
		 * this function is called to filter suggestion list as user types every character.
		 **/ 
		function edits1(words) {
			...
			for (var ii = 0, _iilen = words.length; ii < _iilen; ii++) {
				...
				for (var i = 0, _len = splits.length; i < _len; i++) {
					...
					if (s[1]) {
						for (var j = 0, _jlen = self.alphabet.length; j < _jlen; j++) {
							inserts.push(s[0] + self.alphabet[j] + s[1]);
						}
					}
				}
		...
		function edits2(words) {
			...
			for (var ii = 0, _iilen = words.length; ii < _iilen; ii++) {
				...
				for (var i = 0, _len = splits.length; i < _len; i++) {
					...
					if (s[1]) {
						if (self.check(s[0] + s[1].substring(1))) {
							deletes.push(s[0] + s[1].substring(1));
						}
					}
				}
				...
				for (var i = 0, _len = splits.length; i < _len; i++) {
					...
					if (s[1].length > 1) {
						if (self.check(s[0] + s[1][1] + s[1][0] + s[1].substring(2))) {
							transposes.push(s[0] + s[1][1] + s[1][0] + s[1].substring(2));
						}
					}
				}
				...
				for (var i = 0, _len = splits.length; i < _len; i++) {
					...
					if (s[1]) {
						for (var j = 0, _jlen = self.alphabet.length; j < _jlen; j++) {
							if (self.check(s[0] + self.alphabet[j] + s[1].substring(1))) {
								replaces.push(s[0] + self.alphabet[j] + s[1].substring(1));
							}
				...
				for (var i = 0, _len = splits.length; i < _len; i++) {
					...
					if (s[1]) {
						for (var j = 0, _jlen = self.alphabet.length; j < _jlen; j++) {
							if (self.check(s[0] + self.alphabet[j] + s[1])) {
								inserts.push(s[0] + self.alphabet[j] + s[1]);
							}
						}
					}
		...
		/**
		 * this function is called to check if the word exist in dictionary or not
		 **/
		function known(words) {
		 ...
		 /**
		  * this function is called to suggest correct word.
		  **/
