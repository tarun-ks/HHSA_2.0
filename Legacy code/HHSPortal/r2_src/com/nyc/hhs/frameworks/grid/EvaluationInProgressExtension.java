 ...
 * 
 */
public class EvaluationInProgressExtension implements DecoratorInterface
{
	/**
	 * The Method will check if there is any Proposal in state Non Responsive ,
	 * will display Evaluation in progress as underscore else will display exact
	 * value
	 * <ul>
	...
	public String getControlForColumn(Object aoEachObject, Column aoCol, Integer aoSeqNo) throws ApplicationException
	{
		...
		if (aoCol.getColumnName().equalsIgnoreCase(HHSConstants.EVALUATION_IN_PROGRESS))
		{
			...
			{
				lsControl = loEvaluationBean.getEvalutionsInProgress().toString();
			}
		}
		return lsControl;
	}
