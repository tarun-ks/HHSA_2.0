...
public class LinkTag extends BodyTagSupport
{
	...
	public int doStartTag()
	{
		...
		try
		{
			Link loLink = new Link(href, paramName, paramValue, onClick);
			...
			{
				loColumnTag.setElementType(loLink);
			}
		}
		catch (Exception aoException)
		{
			LOG_OBJECT.Error("Error occured while generating Link Tag", aoException);
		}
		return (SKIP_BODY);
	}
}
