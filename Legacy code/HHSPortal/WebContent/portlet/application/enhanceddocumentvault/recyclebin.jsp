	...
	<ul></ul>
			...
			<div id="documentVaultGrid" class="tabularWrapper tabularWrapperDocumentVault">
				<st:table objectName="documentList"  cssClass="heading"
					...
					<st:property toolTip = "Select all" headingName="<input type='checkbox' onchange='checkAll(this)' name='chkAll'disabled='disabled'/>" columnName="documentId" align="center" size="5%" >
						<st:extension decoratorClass="com.nyc.hhs.frameworks.grid.RecycleBinExtension" />
					</st:property>
					</c:if>
					</c:if>
					<c:if test="${fn:length(documentList) ne 0 }">
					<c:if test="${check ne 'R'}">
					...
					<st:property headingName="Deletion Date" columnName="date"
				...
				<c:choose>
					...
					<c:otherwise>
						<div class="noRecord" id="noDocumentDiv">The recycle bin is empty</div>
					</c:otherwise>
</c:choose>
<input type="hidden" id="readOnlyCheckForRecycleBin" name = "readOnlyCheckForRecycleBin" value="<%= session.getAttribute("permissionType") %>"/>
