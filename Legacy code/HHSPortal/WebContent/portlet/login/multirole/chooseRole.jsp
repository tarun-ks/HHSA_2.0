...
<script type="text/javascript">
	///** start  QC 8914 read only role R 7.2 **/ - Switch to Read Only Role screen /Overlay
	function submitApplication(){
		pageGreyOut();
		document.chooseRole.submit();
	}
	
	//This method is used to switch Role for Multiple Organization.
	function switchRole(){
		pageGreyOut();
		document.chooseRole.action = document.chooseRole.action	+ 'Overlay';
		var options = {
		...
		pageGreyOut();
		$(document.chooseRole).ajaxSubmit(options);
		return false;
	}
	...
	function onReadyMethod(){
		...
		if($(".alert-box-switchRoleDiv").is(":visible")){
			leftToSubtract = $(".alert-box-switchRoleDiv").offset().left;
		}
		$("#optionsBox").css({
		...
		$(".alert-box").find('#cancelSwitchRole').unbind("click").click(function() {
...
<!--NYC Header Ends-->
	...
	<portlet:actionURL var="chooseRoleUrl" escapeXml="false">
		<portlet:param name="submit_action" value="multiRole" />
	</portlet:actionURL>
<div id="selectRoleDiv" >
	<form action="${chooseRoleUrl}" method="post" id="chooseRole" autocomplete="off" name="chooseRole">
		<div class="notify"> 
			<c:choose>
		...
		<div id="selectRoleContainer">
			<c:choose>
				<c:when test="${launchOverlay eq 'true'}">
					<div class="tabularCustomHead" style="color:#1A6BB4";>.</div>
				</c:when>
			</c:choose>
				...
				<span>
					<c:choose>
						<c:when test="${launchOverlay ne 'true'}">
							<p>Please click on the arrow below to use the drop down to select your system view. 
							<br />If you want to log into a different role after your selection below, you can click the Switch Role icon in the top right corner.</p>
						</c:when>
						<c:otherwise>
							<p>Please click on the arrow below to use the drop down to select your system view.
							<br />
				...
				<div class="selectRoleBody">
							...
							<tr>
								<td class="ddcombo_td1">
									<div class="ddcombo_div4" style="background: url(&quot;../framework/skins/hhsa/images/transparent_pixel.gif&quot;) repeat scroll 0% 0% transparent;">
										...
										<input type="hidden" name="roleIdKey" id="roleIdKey"/>
									</div>
								</td>
					...
					<div style="display:none; position: absolute; width: 271px;" class="ddcombo_results" id="optionsBox">
						<ul id= "dropdownul" style="max-height: 180px; overflow: auto;">
							<c:forEach items="${roleDetailsMap}" var="entry">
						...
						<c:choose>
							<c:when test="${launchOverlay ne 'true'}">
								...
								<!-- start QC 9205 R 8.0.0 Imternal SAML 
									<input type="button" value="Cancel" type="reset" class="graybtutton" onclick="location.href='${pageContext.servletContext.contextPath}/portal/hhsweb.portal?_nfpb=true&_pageLabel=portlet_hhsweb_portal_login_page&_nfls=false&logout=logout&siteminderLogout=siteminderLogout&app_menu_name=logout_icon'"/>
								-->	
									<input type="button" value="Cancel" type="reset" class="graybtutton" onclick="location.href='${pageContext.servletContext.contextPath}/portal/hhsweb.portal?_nfpb=true&_pageLabel=portlet_hhsweb_portal_login_page&_nfls=false&logout=logout&app_menu_name=logout_icon'"/>
							   <!-- end QC 9205 R 8.0.0 Imternal SAML  -->
							</c:when>
							<c:otherwise>
								<div class="buttonholder" style="margin-right: 20px;">
									<input type="button" value="Cancel" name="cancel" id="cancelSwitchRole" class="graybtutton" />
						</c:otherwise>
						</c:choose>
						<!-- QC 9092 specify class nycMain.css.button for login button to prevent override with different css parameters -->
						<input value="Login" type="button" class="nycMain.css.button" id="login" name="login" onclick="submitApplication();" disabled="disabled"  />
					</div>
				</div>
			</div>
		</div>
