...
<portlet:resourceURL var="fromStep3Sharing" id="fromStep3Sharing" escapeXml="false"></portlet:resourceURL>
<portlet:resourceURL var="backtoStep2Sharing" id="backtoStep2Sharing" escapeXml="false"></portlet:resourceURL>
...
function onReady(){
		...
		splitPreviousScreenValues('<%=request.getAttribute("agencySet")%>');
...
<form id="share2" action="" method ="post" name="share2">
	<input type="hidden" name="proNameString" id="proNameString">
	<input type="hidden" id="fromStep3SharingForm" value="${fromStep3Sharing}"/>
	<input type="hidden" id="backtoStep2SharingForm" value="${backtoStep2Sharing}"/>
		...
		<!--End of R4 Document Vault changes -->
			<div class="wizardTabs" style='width:800px;'>
					...
					<p>If you would like to grant Provider access to your documents, you can use the search below to find
					...
					the "Next" button.</p>
					<br/>
					<p>If you do not want to grant Provider access to your documents, click the "Next" button now.</p>
					<div class='hr'></div>
					<div class="formcontainer">
						<input type="text" name="provName" class='selectagency-align' id="provName" style='width:400px';/><input type="button" value="+ Add Provider" title="+ Add Provider" class="button selectagency-align" name="addProvider" id="addProvider" onclick='populateTable("","^PROVIDER")'/>
						
						<div  class="tabularWrapper" style="height: 250px !important; overflow: auto;">
						<b>Granting access to the following:</b>
						
							<table border="1" id='mytable' style='width:fixed';>
							</table>
						</div>
