...
public class PreprocessingNT322 implements PreProcessingNotificationAlert
{

	private static final LogInfo LOG_OBJECT = new LogInfo(PreprocessingNT322.class);
			...
			throws ApplicationException
	{
		...
		try
		{
			loUserEmailList = (List<UserEmailIdBean>) DAOUtil.masterDAO(aoMyBatisSession, loUserGroupMap,
					ApplicationConstants.MAPPER_CLASS_NOTIFICATION_MAPPER, "getUserEmailIdsForBulkNotification",
					HHSConstants.JAVA_UTIL_HASH_MAP);

		}
		catch (ApplicationException aoExp)
		{
			LOG_OBJECT.Error("Error occurred while PreprocessingNT_AMIT_FSED:", aoExp);
		}
		catch (Exception loExp)
		{
			LOG_OBJECT.Error("Error occurred while PreprocessingNT_AMIT_FSED:", loExp);
		}
