...
<script type="text/javascript" src="../framework/skeletons/hhsa/js/taskManagement.js"></script>
<script type="text/javascript" src="../resources/js/calendar.js"></script>

<portlet:defineObjects/>

<%--  Begin QC 5446  resourceURL for typeheads--%>
...
String lsMPE1 = "One or more tasks are in locked state.";
...
if(FilterDetails!=null && !FilterDetails.isEmpty()){
	...
	if(null!=(String)FilterDetails.get("TaskOwner")){
		 AssignedToFilter =(String) FilterDetails.get("TaskOwner");
		 if(AssignedToFilter==P8Constants.PROPERTY_PE_VALUE_UNASSIGN)
		 {
...
<!-- Body Wrapper Start -->
	...
	<!-- Body Container Starts -->
			...
			<%if(CommonUtil.getConditionalRoleDisplay(ComponentMappingConstant.TA_S078_PAGE, request.getSession())){%>
				<div id=unfiltered>Listed below are all of the tasks currently assigned to the HHS Accelerator team with the applied filters.</div>
				<div class="hr"></div>
				<!-- Container Starts -->
				<div class="container">
				...
				<input type="button" value="Filter Tasks" title="Filter Tasks" class="filterDocument"  onclick="setVisibility('documentValuePop', 'inline');" />
				...
				<div id="documentValuePop" class='formcontainer'>
					...
					<div class='row'>
						...
						<span class='formfield'>
							<select id="tasktype" name="tasktype" onchange="enableFilter();">
					...
					<!-- **** End QC5446 ****-->
					<!-- Start || Changes done for Enhancement #6280 for Release 3.3.0 -->
					<div class='row'>
						...
						<span class='formfield'>
							<select name="assignedto" id="assignedto">
    							...
    							<option value='Unassigned - Manager'>Unassigned - Manager</option>
					...
					<!-- End || Changes done for Enhancement #6280 for Release 3.3.0 -->
					<div class='row'>
				...
				<input type="hidden" name="reassigntouserText" id="reassigntouserText" />
				<div class="taskreassign" id='reassigndiv' style='<%=displayblock%>'>
				  ...
				  <input type="button" id="reassignId" value="Assign" class="button" title="Assign" onclick='ressignCall()'/>
				  </div>
			</div>
			<!--Filter and Reassign section ends -->
			
			<div class="" style='<%=displayblock%>'>
			<!-- Form Data Starts -->
			<div id="mymain" style='<%=displayblock%>'>
			
			 <!-- Grid Starts -->
			 <%if(null!=totalTasks && Integer.parseInt(totalTasks)>0){%>
			 	<div  class="tabularWrapper"   id='griddiv' style='<%=displayblock%>' >
			 	<st:table  gridName="taskmanagement" objectName="taskItemList" pageSize="<%=sizeofpage%>" cssClass="heading"
					alternateCss1="evenRows" alternateCss2="oddRows" defaultSort="moDateCreated" sortType="DESC">
						...
						<st:property headingName="BA Status" sort="true" columnName="msBaStatus"
					align="right" size="10%"  />
					<!--R7.3 QC 9003 : created a new Column "SA Status" on the for the "Tasks assigned to Staff and "Tasks assigned to Staff" and sort on it  -->
					<st:property headingName="SA Status" columnName="msStatus"
				...
				<% } %>	
						...
						<st:property headingName="Action" columnName="msStatus"
							align="right" size="18%" >
							<st:extension decoratorClass="com.nyc.hhs.frameworks.sessiongrid.ReserveJobExtension" />
						</st:property>
				</st:table>
				</div>
			   	<!-- Grid Ends -->
			...
			<input type="hidden" name="appId" value="<%=(String)request.getAttribute("appId")%>"> 
			<script>
...
$(document).ready(function() {
	...
	document.getElementById('assignedto').value="<%=AssignedToFilter%>";
	enableFilter();
...
function ressignCall(){
	...
	else if(ManagerError!="")
	{
		...
		document.myTaskMform.action=document.myTaskMform.action+'&next_action=assignTask';
