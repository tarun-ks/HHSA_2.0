...
$(document).ready(function() {
	...
	if(document.getElementById("profiltercategory").value!="")
	{
 	...
 	if('null' != '<%=request.getAttribute("message")%>' 
 			&& '<%=request.getAttribute("messageType")%>' != 'confirmation' 
 			&& '<%=request.getAttribute("messageType")%>' != 'Draft'
 			){
	...
	if('<%=request.getAttribute("messageType")%>' == 'confirmation')
	{
		...
		$(".nodelete").unbind("click").click(function(){
			document.provform.action = provFormAction+'&documentId='+'<%=request.getAttribute("documentId")%>'+'&removeNavigator=true&next_action=<%=ApplicationConstants.CANCEL_DELETE%>';
			document.provform.submit();
		});
	}else if('<%=request.getAttribute("messageType")%>' == 'Draft')  
	{
...
$('#deleteDoc, #deleteDoc1').click(function() {
	pageGreyOut();
	$("#next_action").val('deleteDocument');
	document.provform.action = provFormAction+'&documentId='+deleteDocumentId+'&removeNavigator=true';
	// pageGreyOut added to grey out screen while deleting document to fix defect 3607 for release 2.7.0 
	pageGreyOut();
	document.provform.submit();
});
...
$('#nodeleteDoc, #nodeleteDoc1').click(function() {
	pageGreyOut();
	// Start of R4 Document Vault changes : Ajax submit instead of form submit
	document.provform.action = provFormAction+'&documentId='+deleteDocumentId+'&removeNavigator=true&next_action=<%=ApplicationConstants.CANCEL_DELETE%>';
			...
			success: function(responseText, statusText, xhr ) 
					{
						...
						$(".overlay").launchOverlay($(".alert-box-sharedoc"), $(".exit-panel"), "850px", "550px", "onReady");
						//$.unblockUI();
						removePageGreyOut();
					},
					error:function (xhr, ajaxOptions, thrownError)
	...
	<form id="myform" action="<portlet:actionURL/>" method ="post" name='provform'>
		...
		<!-- Body Container Starts -->
			...
			<% if(CommonUtil.getConditionalRoleDisplay(ComponentMappingConstant.DV_S030_SECTION, request.getSession())){%>
				 ...
				 <!--Start of R4 Document Vault changes : Displayed text changed from 'Un-Share by Provider' to 'Un-Share by Organization' -->
			...
			<!-- Popup for Filter Task Starts -->
				<div id="documentValuePop" class='formcontainer providerFilter'>
					...
					<div class='row'>
						...
						<span class='formfield'>
							<select id = "filtertype" name="doctype_city" class="input">
					...
					<div class="buttonholder">
						...
						<input type="button" value="Filter" title="Filter" id="filter" onclick="return displayFilter(provFormAction)"/>
					</div> 
				</div>
