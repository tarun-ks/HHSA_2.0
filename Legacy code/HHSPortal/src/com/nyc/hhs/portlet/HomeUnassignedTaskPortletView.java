...
/**
 * This controller Class is used to Get all the Unassigned Task count for Task
 * name Business Application Tasks,Service Application Tasks,Withdrawal Request
 * - Business Application Tasks, Withdrawal Request - Service Application Tasks
 * and Contact Us Tasks.
 * 
 */

public class HomeUnassignedTaskPortletView extends AbstractController implements ResourceAwareController
{
	private static final LogInfo LOG_OBJECT = new LogInfo(HomeUnassignedTaskPortletView.class);
	...
	protected ModelAndView handleRenderRequestInternal(RenderRequest aoRequest, RenderResponse aoResponse)
	{
		...
		{
			LOG_OBJECT.Error("Exception in HomeUnassignedTaskPortletView Controller", aoEx);
			aoRequest.setAttribute(ApplicationConstants.ERROR_MESSAGE, "Error occurred while processing your request");
		}
		ModelAndView loModelAndView = new ModelAndView(ApplicationConstants.HOME_UNASSIGNED_TASK_FINAL);
	...
	public ModelAndView handleResourceRequest(ResourceRequest aoRequest, ResourceResponse aoResponse)
	{
		...
		catch (ApplicationException aoAppEx)
		{
			...
			LOG_OBJECT.Error("Application Exception in HomeUnassignedTaskPortletView Controller", aoAppEx);
	...
	/**
	 * This Method fetches the actual task count for different task type for
	 * Unassigned Tasks.
	 * 
	...
	@SuppressWarnings(
	{ "unchecked" })
	private void getTaskCountMap(ResourceRequest aoRequest, P8UserSession aoUserSession) throws ApplicationException
	{
		...
		loChannel.setData(ApplicationConstants.TASK_OWNER_NAME, ApplicationConstants.UNASSIGNED);
		...
		loTaskCount
				.setBrAppMgrTaskCount(null == loReqPropsMap.get(ApplicationConstants.BUSINESS_APP_MANAGER) ? ApplicationConstants.INT_ZERO
						: loReqPropsMap.get(ApplicationConstants.BUSINESS_APP_MANAGER));
		loTaskCount
				.setSrAppMgrTaskCount(null == loReqPropsMap.get(ApplicationConstants.SERVICE_APP_MANAGER) ? ApplicationConstants.INT_ZERO
						: loReqPropsMap.get(ApplicationConstants.SERVICE_APP_MANAGER));
		//QC 9587 R 8.10.0 disable Contact Us task
		//loTaskCount
		...
		aoRequest.setAttribute(ApplicationConstants.TASK_COUNT_UNASSIGNED_BEAN, loTaskCount);
	}
	...
	@SuppressWarnings(
	{ "unchecked", "rawtypes" })
	private void getAgencyTaskCountMap(ResourceRequest aoRequest, P8UserSession aoUserSession)
