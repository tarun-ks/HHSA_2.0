...
$(document).ready(function() {
	$("span.label").each(function(){
      ...
      $("input[name='DUNS']").fieldFormatter("XXXXXXXXX");
    //defines the action performed when a button is clicked
      $("input[type='button'][id^='buttonElt']").click(function(){
    	  	if($(this).attr("name")=="Cancel"){
    	  	     var $self=$(this);
    	            ...
    	            if(!isSame && lastData != null){
    	                  ...
    	                  .dialog({
    	                        ...
    	                        buttons: {
    	                              OK: function () {
    	                            	  location.href = $("#contextPathSession").val()+"/portal/hhsweb.portal?_nfpb=true&_pageLabel=portlet_hhsweb_portal_page_organizationPprofile&_nfls=false&app_menu_name=header_organization_information&section=basics&subsection=questions&next_action=showquestion&action=orgBasicInformation&fb_formName=OrgProfile";
    	                                    $(this).dialog("close");
    	                              },
    	                              Cancel: function () {
    	                                    $(this).dialog("close");
    	  	...
    	  	}else if($(this).attr("name")!="updateLegalName" && $(this).attr("name")!="updateAccountingPeriod"){
                  if($(this).attr("name") == "save_next" || $(this).attr("name") == "save" ){
                      ...
                      document.OrgProfile.action = actionUrl + '&isAjaxCall=true&next_action=' + buttonName;
                      var options = {// sets the erorr message to be displayed
              				success : function(responseText, statusText, xhr) {
              					...
              					if($("input[name='OLN']").val().length == 0){
              						$("input[name='OLN']").closest('table').find("div[id^='buttonElt']").html("! This field is required").addClass("error");
              						errorExist = true;
              					}else if($("input[name='OLN']").val().length < 3){
              						$("input[name='OLN']").closest('table').find("div[id^='buttonElt']").html("! This field should be minimum 3 character long").addClass("error");
              						errorExist = true;
              					}else if($("input[name='OLN']").val().length > 100){
              						$("input[name='OLN']").closest('table').find("div[id^='buttonElt']").html("! This field should be maximum 100 character long").addClass("error");
              						errorExist = true;
              					}else{
              						lsResult =  "^[0-9a-zA-Z !@\\\#\\\$%^\\\&*()-_+=|\\\\{}\\\[\\\];:\\\"\\\'<>,.?\\\/`~\xA7\n\r\t ]+$";
          							var re = new RegExp(lsResult);
          					        if(!re.test($("input[name='OLN']").val())){
          					        	$("input[name='OLN']").closest('table').find("div[id^='buttonElt']").html("! Please enter valid text").addClass("error");
                  						errorExist = true;
          							}
          					        else{
          					        	$("input[name='OLN']").closest('table').find("div[id^='buttonElt']").html("").removeClass("error");
          					        }
              					}
              					if($("select[name='Frommonth']").prop("selectedIndex") == 0){
              						$("select[name='Frommonth']").closest('table').find("div[id^='buttonElt']").html("! This field is required").addClass("error");
              						errorExist = true;
              					}else{
              						$("select[name='Frommonth']").closest('table').find("div[id^='buttonElt']").html("").removeClass("error");
              					}
              					if(!errorExist){
      ...
      });//This function sets the value for the month selected
      ...
      // This function is invokes on the click of updateLegalName button 
      //and executes the appropriate action
      $("input[name='updateLegalName']").click(function(){
            $("#cancelButton").unbind('click').click(function(){
            	var isSame = true;
            	if($("#newLegalName").val().length > 0 || $("#newLegalNameReason").val().length > 0 )
            		isSame = false;
                 if(!isSame){
                	 $('<div id="dialogBox"></div>').appendTo('body')
 					.html('<div><h6>You have unsaved data. <br />If you would like to leave this screen without saving your data, click <b>OK</b>. <br/>If you would like to save your data, click <b>Cancel</b> and save your data.</h6></div>')
            ...
            $("#newLegalNameForm").validate({
                  rules: {
                        newLegalName: {	passSpecialChar: true,
                        				required: true,
                        				maxlength: 100,
                        				minlength: 3 ,
                        				allowSpecialChar: ["A", " !@\\\#\\\$%^\\\&*()-_+=|\\\\{}\\\[\\\];:\\\"\\\'<>,.?\\\/`~\xA7"]},
                        newLegalNameReason: {passSpecialChar: true,
                        					required: true,
                        					maxlength: 250,
                        					allowSpecialChar: ["A", " !@\\\#\\\$%^\\\&*()-_+=|\\\\{}\\\[\\\];:\\\"\\\'<>,.?\\\/`~\xA7\n\r\t"]}
                  ...
                  messages: {
                        newLegalName: {
                            ...
                            maxlength: "! Organization Legal Name should be maximum 100 character long",
                            minlength: "! Organization Legal Name should be minimum 3 character long",
                        ...
                        newLegalNameReason: {
                            ...
                            allowSpecialChar: "! Your response contains invalid characters. Please re-type your response or visit Online Help for instructions on how to Copy and Paste as plain text."} 
                  },
                  submitHandler: function(form){
                	  pageGreyOut();
                	  $("#submitRequest").attr("disabled", true);
                	  hhsAjaxRender(null, document.getElementById("newLegalNameReason"), "", form.action, "afterOrgLegalNameUpdate");
  				  }
            });

      });// binds the action for the click of update accounting period
      $("input[name='updateAccountingPeriod']").unbind('click').click(function(){
            $("#cancelButton1").unbind('click').click(function(){
            	var isSame = true;
            	if($("#effectiveYear").val().length > 0
            			|| $("#newFrom>select[name='Frommonth']").prop("selectedIndex") > 0 
            			|| $("#newTo>select[name='Tomonth']").prop("selectedIndex") > 0 )
            		isSame = false;
                 if(!isSame){
                	 $('<div id="dialogBox"></div>').appendTo('body')
 					.html('<div><h6>You have unsaved data. <br />If you would like to leave this screen without saving your data, click <b>OK</b>. <br/>If you would like to save your data, click <b>Cancel</b> and save your data.</h6></div>')
            ...
            $(".overlay").launchOverlay($(".alert-boxAccountingPeriod"), $(".exit-panel"), "595px");
            $(".exit-panel").click(function(){
            	cleanFormData();
            });
            ...
            $("#newTo>select[name='Tomonth']").width("70").find("option").eq(0).attr('selected', 'selected').html("Month");
            $("#newTo>select[name='Tomonth']").attr("disabled", false);
            $("#newFrom>select[name='Frommonth']").width("70").find("option").eq(0).attr('selected', 'selected').html("Month");
            $("#newFrom>select[name='Frommonth']").attr("disabled", false);             
            $("#newPeriodForm").validate({
                  ...
                  messages: {
                        effectiveYear: {required: "! This field is required",
                        				number: "! The format for this field must be YYYY",
                        				minlength:"! The format for this field must be YYYY"
                                                },
                        Frommonth: {noneSelected: "! This field is required"},
                        Tomonth: {noneSelected: "! This field is required"}
                  },
              	submitHandler: function(form){
              		hhsAjaxRender(null, document.getElementById("effectiveYear"), "errorMessageToBeDisplayed", form.action, "afterAccountPeriodUpdate");
				},
				errorPlacement: function(error, element) {
			         element.parent().parent().parent().find("span.error").html(error.outerHTML());
				}
            });
      });
      //Fix for Defect #2832, Hiding DBA field from Organization Info Page
      document.getElementById('fb_divBl0ck20').style.display = "none";
});
//theis method closes the overlay and clearsthe form
function afterOrgLegalNameUpdate(){
	$("#submitRequest").attr("disabled", false);
	$(".overlay").closeOverlay();
    cleanFormData();
    removePageGreyOut();
}
//This method sets the value of from and to months
function afterAccountPeriodUpdate(){
	if($("#errorMessageToBeDisplayed").children().size()==0){
		 $("select[name='Frommonth'] option").eq($("#newFrom>select[name='Frommonth']").prop("selectedIndex")).attr('selected', 'selected');
         $("select[name='Tomonth'] option").eq($("#newTo>select[name='Tomonth']").prop("selectedIndex")).attr('selected', 'selected');
         $("input[name='Frommonth']").val($("#newFrom>select[name='Frommonth']").val());
         $("input[name='Tomonth']").val($("#newTo>select[name='Tomonth']").val());
         $(".overlay").closeOverlay();
         cleanFormData();
	}
}
...
// This method checks and display the element selected
...
function cleanFormData(){
	$("#errorMessageToBeDisplayed").html("");
	$("#newLegalName").val("");
	$("#newLegalNameReason").val("");
	$("#effectiveYear").val("");
}
