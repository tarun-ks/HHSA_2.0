	    ...
	    <div class="container"> 
        	...
        	<div id="mymain">
                ...
                <div class="row" id="title"> <span class="label"><span class="required">*</span>Title:</span>
	                <span class="formfield">
		                <select class="input" id="staffTitle" name="staffTitle" ${readOnlyValue} onchange="resetTitle()">
		                 <option value="-1" id="-1"></option>
		             ...
		             <c:if test="${ceoExist ne null}">
						<div class="individualError" id="ceoError">${ceoExist}</div>
					</c:if>	
                 </div>
                 <input type="hidden" id="oldTitle" name="oldTitle" value="<%=staffDetails.getMsStaffTitle()%>"></input>
	           ...
	           <c:if test="${serviceComments ne null and ! empty serviceComments}">
	            ...
	            <div class="buttonholder">
	                ...
	                '<%=renderRequest.getAttribute("business_app_id")%>','<%=renderRequest.getAttribute("section")%>','<%=renderRequest.getAttribute("subsection")%>','${elementId}')"/>
	                 <c:if test="${!loReadOnlySection}">
	                <input id='saveform' type="submit" value="Save" title="Save" name="saveform"/>
	                </c:if>
	            </div>
            <!-- Form Data Ends --> 
	        </div>
