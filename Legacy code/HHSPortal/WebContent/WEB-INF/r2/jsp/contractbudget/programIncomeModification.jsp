<%--
This JSP file is used for Program Income Tab on Contract Budge Modification Page
--%>
...
<portlet:param name="gridLabel" value="categoryProgramIncomeModification.grid"/>
</c:when>
<c:when test="${oldPIFlag eq 0}">
<portlet:param name="gridLabel" value="programIncomeModificationNew.grid"/>
</c:when>
<c:otherwise>
<portlet:param name="gridLabel" value="programIncomeModification.grid"/>
</c:otherwise>
</c:choose>
<%-- R7 changes end --%>
</portlet:resourceURL>
...
<c:set var="lastRowEditAttribute" value="false"></c:set>
<c:set var="PIexportFileName" value="PROGRAM_INCOME_MODIFICATION_"></c:set>
<c:set var="isPaginationValue" value="true"></c:set>
...
<%-- set Grid properties from property file --%>
<c:set var="gridColNames"><%=HHSUtil.getHeader("programIncomeModification.grid")%></c:set>
<c:set var="gridColProp"><%=HHSUtil.getHeaderProp("programIncomeModification.grid")%></c:set>
<c:set var="subGridColProp"><%=HHSUtil.getSubGridProp("programIncomeModification.grid")%></c:set>

<%-- Added in R7 for new Program income grid UI --%>
<c:if test="${oldPIFlag eq 0}">
<c:set var="gridColNames"><%=HHSUtil.getHeader("programIncomeModificationNew.grid")%></c:set>
<c:set var="gridColProp"><%=HHSUtil.getHeaderProp("programIncomeModificationNew.grid")%></c:set>
<c:set var="subGridColProp"><%=HHSUtil.getSubGridProp("programIncomeModificationNew.grid")%></c:set>
</c:if>
...
<%-- For showing PI grid in other budget categories--%>
<c:when test="${(oldPIFlag eq 0) and !(param.entryTypeId eq null)}">
<c:set var="gridColNames"><%=HHSUtil.getHeader("categoryProgramIncomeModification.grid")%></c:set>
<c:set var="gridColProp"><%=HHSUtil.getHeaderProp("categoryProgramIncomeModification.grid")%></c:set>
