...
$(document).ready(function() {
	...
	if((status == 'Draft' || sectionStatus == 'Returned for Revision') && orgType == 'provider_org'){
		$("#dropdownElt11337346065017dIi15v_select").attr('disabled', false);  
	}
	else
		{
		$("#dropdownElt11337346065017dIi15v_select").attr('disabled', true);  
		$("#dropdownElt1341669712592dIi15v_select").attr('disabled', true); 
		}
...
// This function sets the height of the span label	
	$("span.label").each(function(){
      ...
      // This function checks whether save or save next is clicked and executes appropriate action  
      $("input[type='button'][id^='buttonElt']").click(function(){
            ...
            if($(this).attr("name")!="updateLegalName" && $(this).attr("name")!="updateAccountingPeriod"){
                  if($(this).attr("name") == "save_next" || $(this).attr("name") == "save" ){
                	  ...
                	  if($("#dropdownElt11337346065017dIi15v_select").val() != ' ' && corporateStructure != $("#dropdownElt11337346065017dIi15v_select").val() && filingSubSectionFlag == 'true')
            		  {
            		  $(".overlay").launchOverlay($(".alert-confirm-override"), $(".exit-panel"), "800px", null);
            		  }
                	  else
            		  {
                        ...
                        var options = {
                				...
                				success : function(responseText, statusText, xhr) {
                					...
                					// This function sets the error messsage
      ...
      // This function is invoked on changing of the select tag and calls checkAndDisplay function 
      //passing the node as an parameter 
      ...
      //This function is invoked on clicking of the update legal name button
      ...
      });//binds the action to the update accounting period button
...
//This function is used to close overlay
 ...
 function confirmOverride1() {
	  document.Basic.action = actionUrl + '&isAjaxCall=true&next_action=' + buttonName + '&subSectionFilingFlag=' + filingSubSectionFlag;
	  var options = {
   		//function finds the elements belonging to class overlaycontent
			success : function(responseText, statusText, xhr) {
				$(".alert-confirm-override").closeOverlay();
				var $response = $(responseText);
				var data = $response.contents().find(".overlaycontent");
				var errorExist = false;
				// This function sets the error messsage
