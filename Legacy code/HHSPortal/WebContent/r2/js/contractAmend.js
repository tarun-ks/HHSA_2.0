// This function called On click of save
	function onSaveClick() {
		...
		var urlAppender = $("#saveContractBudgetAmendUrl").val();
		jQuery.ajax({
			type : "POST",
			url : urlAppender,
			data : v_parameter,
			success : function(result) {
				if(result.indexOf("amendFYBudget")!=-1){
		        	$("#assignAdvanceId1").html(result);
	...
	function openOverlay() {
		...
		$("#successGlobalMsg").hide();
		pageGreyOut();
	var jspName = "submitCBAmendmentConfirmation";
	...
	var urlAppender = $("#submitContractBudgetAmendOverlayUrl").val();
	jQuery.ajax({
		type : "POST",
		url : urlAppender,
		data : v_parameter,
		...
		jQuery.ajax({
			...
			success : function(e) {
				...
				if($('#existingBudget').val() == 0){
					subRefreshFringGridHeader($("#val5" + e['SubBudgetId']).html().replace('(','').replace(')','').replace('%',''),e['SubBudgetId']);
				}
				return false;
			},
	...
	function refreshNonGridContractedServicesData(subBudgetIdVal){
		...
		var urlAppender = $("#getCallBackDataAmendment").val();
		jQuery.ajax({
			type : "POST",
			url : urlAppender,
			data : v_parameter,
	...
	$.jgrid.extend({
		...
		if (iRow.indexOf('_newrecord_contractServices') !== -1){
        ...
        }else if (iRow.indexOf('_newrecord_rent') !== -1){
			totalColumn = getSubGridIds(iRow);
			var tempStringlen = totalColumn.split(',');
	...
	function refreshNonGridDataContBudModOTPS(subBudgetIdVal){
		...
		jQuery.ajax({
			...
			data : v_parameter,
			success : function(e) {
				$("#fyBudgetModOTPS"+e['SubBudgetId']).html(e['keyFYBudgetModOTPS']).jqGridCurrency();
	...
	**/
	function refreshFringGridHeader(subBudgetIdVal){
		...
		subRefreshFringGridHeader(cellValue,subBudgetIdVal);
	}
	...
	* Added in Release 6
	**/
	function subRefreshFringGridHeader(cellValue,subBudgetIdVal){
		if(cellValue == 0){
			cellValue = "0.00%";
		}
		else{
		...
		var fringeObj = $('#table_fringeBenefitsGrid-'+subBudgetIdVal+'_>tbody>tr:eq(1)>td:eq(3)');
